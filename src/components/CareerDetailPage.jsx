...
function ApplicationForm({ roleTitle }) {
  ...
  const lbl = "block text-[10px] font-bold text-slate-400 mb-1.5 uppercase tracking-wider";
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3.5">
      ...
      <div>
        <label className={lbl}>Resume <span className="normal-case text-slate-400">(Google Drive URL)</span></label>
      ...
      <div>
        <label className={lbl}>LinkedIn URL <span className="normal-case text-slate-400">(optional)</span></label>
      ...
      <div className="flex items-start gap-2 pt-0.5">
        <svg className="w-3.5 h-3.5 text-slate-400 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
...
function CareerDetailPage({ roleId }) {
  ...
  if (!role) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center px-6 py-20">
        <p className="text-slate-400 text-sm mb-4">Role not found.</p>
        <button onClick={() => navigateTo('/careers')} className="text-sm font-semibold text-violet-600 hover:underline">
          Back to Careers
        </button>
      </div>
    );
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      >
        <div className="absolute inset-0 bg-gradient-to-b from-white/70 to-white pointer-events-none" />
        <div className="relative max-w-6xl mx-auto px-6 sm:px-8 xl:px-12 py-10 sm:py-14">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }}>
            ...
            <p className="text-base sm:text-lg text-slate-500 max-w-2xl mb-7 leading-relaxed">{role.tagline}</p>
      ...
      <div className="max-w-6xl mx-auto px-6 sm:px-8 xl:px-12 py-8 sm:py-12">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8 lg:gap-12 items-start">
          ...
          >
            {/* Quick Insights */}
            <section>
              <h2 className="text-base font-bold text-slate-900 mb-4 pb-3 border-b border-slate-100">Quick Insights</h2>
            ...
            {/* Why Join */}
            <section>
              <h2 className="text-base font-bold text-slate-900 mb-4 pb-3 border-b border-slate-100">Why Join This Role?</h2>
              <div className="bg-violet-50 border border-violet-100 rounded-2xl p-5">
                <p className="text-[15px] text-violet-900 leading-relaxed">{role.whyJoin}</p>
              </div>
            </section>

            {/* About */}
            <section>
              <h2 className="text-base font-bold text-slate-900 mb-4 pb-3 border-b border-slate-100">About the Role</h2>
              <p className="text-[15px] text-slate-600 leading-relaxed">{role.about}</p>
            </section>

            {/* Responsibilities */}
            <section>
              <h2 className="text-base font-bold text-slate-900 mb-4 pb-3 border-b border-slate-100">Responsibilities</h2>
            ...
            {/* Requirements */}
            <section>
              <h2 className="text-base font-bold text-slate-900 mb-4 pb-3 border-b border-slate-100">Requirements</h2>
              <ul className="space-y-3">
            ...
            {/* Hiring Process */}
            <section>
              <h2 className="text-base font-bold text-slate-900 mb-5 pb-3 border-b border-slate-100">Hiring Process</h2>
              <div className="space-y-4">
                {HIRING_STEPS.map((s, i) => (
                  <div key={i} className="flex gap-4 items-start">
                    <div className="shrink-0 w-8 h-8 rounded-full bg-slate-900 text-white flex items-center justify-center text-xs font-bold">{s.step}</div>
            ...
            {/* Tools */}
            <section>
              <h2 className="text-base font-bold text-slate-900 mb-4 pb-3 border-b border-slate-100">Tools You'll Use</h2>
              <div className="flex flex-wrap gap-2">
                {tools.map((tool) => (
                  <span key={tool} className="text-sm font-medium text-slate-700 bg-slate-50 border border-slate-200 px-4 py-2 rounded-full hover:border-violet-200 hover:bg-violet-50 hover:text-violet-700 transition-all duration-200">
                    {tool}
                  </span>
                ))}
              </div>
            ...
            {/* Perks */}
            <section>
              <h2 className="text-base font-bold text-slate-900 mb-4 pb-3 border-b border-slate-100">Perks & Benefits</h2>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {perks.map((perk) => (
                  <div key={perk} className="flex items-center gap-2 text-sm text-slate-600 bg-slate-50 rounded-xl px-3 py-2.5 border border-slate-100">
